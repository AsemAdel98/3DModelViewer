...
export class ModelViewerComponent implements OnInit {
  descreption = `A backpack is a versatile, bag designed to be worn on the back and secured with two straps that go over the shoulders. It typically features a large main compartment and various smaller pockets or compartments for organization. Backpacks are made from a variety of materials, including canvas, nylon, polyester, and leather, and come in various sizes and styles to suit different needs.`
 ...
 splitDescription() {
  if (this.descreption) {
    const maxLength = Math.ceil(this.descreption.length / 2);
    this.firstPart = this.descreption.slice(0, maxLength);
    this.secondPart = this.descreption.slice(maxLength);
  }
